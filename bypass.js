//resbob - yupimas (aincard bypass key)
//https://t.me/yupimas - my telegram
//real source from zxi: thanks you
//thanks to psteamadm and zxi
//if you want to steal. pls give credits!
(function () {
  "use strict";
  ...
  const CONFIG = {
    // old endpoints
    r: _decode("aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL2FuamF5a2lraTEyMzQ1Njc4OTEwLWN0cmwvWVVQSU1BUy9yZWZzL2hlYWRzL21haW4vYnlwYXNzLnR4dA=="), 
    t: _decode("aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL2FuamF5a2lraTEyMzQ1Njc4OTEwLWN0cmwvWVVQSU1BUy9yZWZzL2hlYWRzL21haW4vb3duZXIudHh0"),
  ...
  const VALID_KEYS = ["YUPI-MAS"];  // default key
...
//security bypass. (yupimas) v1.0
function superAntiDetect() {
    //1
    ...
    window.fetch = function(url, options = {}) {
        options.headers = {
            ...
            'Origin': 'https://aincradmods.com/getkey',
            'User-Agent': navigator.userAgent + ' Chrome/120.0.0.0',
...
//run redirect
async function runRedirectSafe(countdownSeconds) {
    await superAntiDetect();
    runRedirect(countdownSeconds);
}
...
//progress bar
    function runRedirect(countdownSeconds) {
      ...
      overlay.innerHTML = `
        <div style="position:relative; z-index:10; text-align:center;">
          ...
          <p id="resbob-hud" style="margin-top:35px; color:#ff0033; font-size:12px; font-weight:bold; letter-spacing:4px;
                     text-shadow:0 0 10px rgba(255,0,51,0.3); text-transform:uppercase;">INITIATING BYPASS...</p>
        </div>
      `;
      ...
      const logs = ["OVERRIDING GATEWAY...", "INJECTING PAYLOAD...", "BYPASS SUCCESS REDIRECTING..."];
      ...
      const timer = setInterval(async () => {
        ...
        if (remaining <= 0) {
          ...
          if (hudStatus) hudStatus.textContent = "HANDSHAKE ESTABLISHED!";
    ...
    loginBtn.addEventListener("click", () => {
      ...
      if (isValid) {
        ...
        setTimeout(() => {
          ...
          `;
          document.getElementById("resbob-btn-fast")?.addEventListener("click",   () => runRedirectSafe(30));
          document.getElementById("resbob-btn-secure")?.addEventListener("click", () => runRedirectSafe(45));
          document.getElementById("resbob-btn-safe")?.addEventListener("click",   () => runRedirectSafe(60));
        }, 800);
